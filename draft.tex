...
% In the context of physics, the word ``plasma'' is used to designate a gas whose degree
% of ionization is such that the interaction between its components is dominated by the electric
% and magnetic forces generated by the presence and movement of the negatively and positively
% charged particles.
%
% In the context of physics, the word ``plasma'' is used to designate a gas whose
% degree of ionization is such that its dynamics is dominated by the forces of
% electric and magnetic nature of the charged particles, overcoming the influence
% of the collisions between neutral particles.
%
% In the context of physics, the word ``plasma'' is used to designate a gas whose
% degree of ionization is such that the interactions involving forces of electric
% and magnetic nature of the charged particles, overcome the influence of the
% collisions between neutral particles, in the overall dynamics of the gas.
%
...
%
% A plasma is an ionized gas in which the charged particle population is sufficiently
% large so the forces of electric and magnetic nature, generated by the charges and
% their movement, overcome the influence of the collisions between neutral particles,
% that dominate the kinetics of neutral gases, on the overall dynamics of the system.
%

Let us consider a given set of atoms or molecules, which is composed of
positively and negatively charged particles, and also neutral components.
One can say, correctly, that such a system is an ionized gas, because of
the presence of charged particles in its composition. However, what can
be said about the dominant dynamics of the ionized gas, since neutral and
charged particles have quite a distinct behavior? Neutral particles,
assuming that the gravitational force is negligible, will move freely
in the medium until one collides with another \cite{klimon}. Therefore,
the interaction between the neutral components is characterized as
short-ranged, since it relies only on collisions between two particles.
Charged particles, on the other hand, are sources of electric fields.
Besides that, their motion generates currents that will induce magnetic
fields. The charged particles dynamics is dominated by the presence
of these fields that can act at distance, interacting and  the
fields of several particles at the same time.


The plasma physics branch of Physics is broad. From life-changing
industrial applications like the plasma processing of integrated
circuits - an essential step of the manufacturing process of the
tiny integrated circuits -, passing through the development of the
telecommunications via radio-frequency, to day-to-day devices
like fluorescent lamps \cite{bell,gurnett2017,chen}. Another
important application of plasmas on Earth

% The first fact that a plasma physics student learns, before even
% understanding the definition of plasma, is that most of the baryonic
% matter in the universe is in the plasma state. This \emph{most} is
% usually estimated to be around $99\%$ \cite{bitt,gurnett2017}. If this
% high estimate has something to do with the fact that our probes and
% telescopes can only directly detect ionized matter, because it emits
% radiation, \cite{chen} it is still a case to be studied.

% In order to understand what is a plasma, one must have to be aware of
% the differences between the dynamics of neutral and fully ionized gases.
% In a neutral gas, assuming that the influence of gravity can be neglected,
% the dynamics is dictated by collisions between two components that are
% very close to each other \cite{klimon}. A fully ionized gas, on the other
% hand, is only composed of charged particles, which are sources of electric
% fields. Besides that, the motion of charged particles generates electric
% currents, that will induce magnetic fields. In such arrangement, one charge
% can interact with several other charges at a considerable distance trough
% the action of the self-generated electric and magnetic fields. Therefore, a
% partially ionized gas is considered a plasma when its degree of ionization
% is such that its dynamics is dominated by the electric and magnetic forces
% of the charged particles. Under such definition, a fully ionized gas is, of
% course, also considered a plasma.

% Plasmas can be classified according with their overall neutrality as
% \emph{neutral} when the density of electrons and ions are the same, and
% \emph{non-neutral} when the densities of the negatively and positively
% charged particles are different, which includes the case of one component
% plasma.



% In the beginning of this chapter it was mentioned a fundamental
% characteristic of a plasma: the \emph{collective processes}.
% Indeed, an ionized gas cannot be called a plasma if its dynamics
% is not predominantly collective. 

% In order to define collective processes, one should have in mind that
% a plasma is a gas that has most of its components in an ionized state.
% For instance, let us consider a hydrogen gas that was heated to the
% point where the electrons were separated from their respective ions.
% What we have now is gas that is composed of charged particles that
% are source of electric fields. Also, the motion of the electrons
% generates electric currents that induce magnetic fields.

On Earth, the plasma is considered an exotic state of matter, associated with the
glowing gas inside fluorescent and neon lamps, microchip manufacturing, research
laboratories, and the promise of cleaner and technically infinite energy source
when scientists and engineers manage to succeed at obtaining a controlled, and
energetically efficient, thermonuclear fusion. Naturally occurring plasmas in our
everyday life are rare, restricted to the occurrence of lightning during storms,
flames and beautiful auroras that are spotted at high latitudes. In the
interplanetary and interstellar space, however, the situation is the opposite.
Starting in the outer layers of Earth's atmosphere, passing through the solar
wind in the interplanetary space, to the stellar interiors and atmospheres,
accretion disks, and molecular clouds it is estimated that something around
$99 \sim 99.9\%$ of the observable matter is in the plasma state \cite{may}.

What defines the plasma state in an ionized gas, is the presence of collective
behavior, which is a consequence of the long-range action of the electric and
magnetic forces that are generated by the charged particles and their movement.
There are other important properties, like Debye shielding and quasineutrality,
that are well described in introductory books \cite{chen,bitt,gurnett2017} and
will not be addressed here. Beyond these fundamental properties, plasmas can
be found in an immense variety of configurations, and this is notably true for
the plasma in space.

%The plasma that permeates the near Earth environment, and our solar system
% has its source in the plasma ejections 

% During the development of this doctoral research, there was always a
% concern about not being restricted to purely theoretical modeling of
% the plasma phenomena. 




% Describing nonlinear processes in plasmas, in
% the kinetic theory level, requires a series of approximations and
% idealized assumptions that, at first glance, it might look like one
% is losing the connection with the real-world physics.

% On Earth, the plasma is considered an exotic state of matter, associated to
% the glowing gas inside fluorescent and neon lamps, microchip manufacturing,
% research laboratories, and the promise of cleaner and technically infinite
% energy source when scientists and engineers manage to succeed at obtaining
% a controlled, and energetically efficient, thermonuclear fusion \cite{bell}.
% Naturally occurring plasmas in our everyday life are rare, restricted to the
% occurrence of lightning during storms, flames, and the beautiful auroras that
% take place at high latitudes. In the interplanetary and interstellar space,
% however, the situation is the opposite. Starting in the outer layers of
% Earth's atmosphere, passing through the solar wind in the interplanetary space,
% to the stellar interiors and atmospheres, accretion disks, and molecular clouds
% it is estimated that something around $99 \sim 99.9\%$ of the observable matter
% in the universe is in the plasma state \cite{may}. Therefore, the study and
% comprehension of the physics of plasma processes is a fundamental branch of
% astrophysics.

% Plasma is a gas in which the atoms -or molecules- are in an ionized state.
% In such condition, long-range electric and magnetic forces determine the
% interactions of the charged particles. This coupling between the variations
% in the electric and magnetic fields and the motion of ions and electrons
% is behind the collective behavior that characterizes the plasma dynamics
% \cite{tsytotp}. The complex processes

% It is well known that the plasma in our solar system is considered as a natural
% laboratory for the study of space plasma. The wide variety of plasma phenomena
% accessible to direct, in situ measurements and highly resolved remote sensing,
% might be scaled to other astrophysical environments. Besides that, theoretical
% models developed to explain phenomena observed in the solar system plasma might
% be extrapolated to describe plasma process occurring in more extreme astrophysical
% systems.


% Astrophysical plasmas are observed in a vast range of densities, temperatures,
% magnetic fields, and scale lengths\footnote{The description of such an extensive
%   list is out of the scope of this text. On that matter, Ref.~\cite{peratt2014}
%   offers a quite complete exposure of space plasma, and space plasma phenomena
%   occurring in all cosmic scales.}.




% With some exotic exceptions, like the super
% dense and degenerate plasma of white dwarfs (which is beyond the applicability
% of our theory), the solar system hosts a large variety of plasma whose properties
...
% interactions of the charged particles. This coupling between the variations
% in the electric and magnetic fields and the motion of ions and electrons
% is the cause of the wide variety of oscillatory processes that characterize
% the plasma dynamics\cite{tsytotp}. 


% In this thesis, we present a theoretical model for the inclusion of single
% particle fluctuations and collisional processes related to electrostatic
% oscillation in rarefied, unmagnetized, non-relativistic and fully ionized
% plasmas.


% The problems in solar physics addressed by our theory are related to three
% specific regions of the wide range of action of the solar plasma: solar activity
% regions \cite{Tigik2016a}, transition region between the chromosphere and solar
% corona \cite{Tigik2017a} and the solar wind \cite{Tigik2017b}. The paper 

% In this appendix we provide a brief discussion about the space plasma phenomena
% that are addressed by our theory. 

% In this text we presented three papers that were published during this Ph.D.
% research, \cite{Tigik2016b,Tigik2017a,Tigik2017b}, and each one of them
% address a different problem in solar physics. There is also another article,
% \cite{Tigik2016a}, which corresponds to the results of the Master's
% dissertation that served as basis for this project.

% Each one of the four papers published during this graduate studies is 
% different solar physics phenomena

% In order to offer a deeper insight on the solar phenomena 
% In this appendix, we make a brief description of the solar processes that are
% being addressed by our theory.
...
% The theory developed during this graduate research makes use of solar physics
% results to contribute on both sides. In \cite{Tigik2016a},
...
% space probes to be confirmed.
 ------------------------------------------------------------------------

% Investigations of the origin and behavior of the solar wind, energetic
% particles, and magnetic fields in the heliosphere and their interaction
% with Earth and other planets, as well as with the interstellar medium.
% Investigations of the physics of magnetospheres, including fundamental
% interactions of plasmas and particles with fields and waves, and coupling
% to the solar wind and ionospheres.
% Investigations of the physics of the terrestrial mesosphere, thermosphere,
% ionosphere, including the coupling of these phenomena to the lower atmosphere
% and magnetosphere.


% % The appearance of sunspots is a sign of
% % solar activity, characterized by solar flares and coronal mass ejections.



% This enhanced flux of energetic particles and radiation have direct impact
% in the \emph{space weather} around Earth, triggering geomagnetic storms
% that 



% Inside the heliosphere, the Sun, the space environment around Earth and the
% interplanetary medium are extensively explored by space probes.
...
% \item   
% \end{itemize}
 % On that regard, a critical aspect being addressed is the response of Earth’s
 % magnetosphere, ionosphere, and atmosphere to solar inputs.
 % Ref. \cite{schrijver2009}


% \usepackage[inline]{enumitem}
% \begin{enumerate*}[label={\alph*)},font={\bfseries}]
% \item very rarefied plasmas, where collisions can be completely neglected,
%   and
% \item extremely fast phenomena, where the quasi-steady state is attained before
%   collisions can affect the dynamics.
% \end{enumerate*}

% , and have
% been a successful approach for describing kinetic oscillatory instabilities
% and micro-turbulent processes



% As pointed in the introduction, the kinetic theory of plasmas is mostly
% concerned with processes that involve waves and oscillatory instabilities.
% Kinetic instabilities depend on the inter-particle displacement and the
% electrostatic and electromagnetic oscillatory modes that arise as a response.
% This kind of physics cannot be described using a macroscopic approach like
% magnetohydrodynamics (MHD), that treat the plasma as magnetized, incompressible
% fluid. A single-particle description is also out of the question because it
% is both mathematically and computationally prohibitive. Statistical mechanics,
% however, suits very well for the problem. But, as the reader could have noticed,
% the kinetic description of plasmas is quite cumbersome and requires a lot of
% approximations to get to any result. This does not mean that kinetic theory is
% incapable to provide a reliable description of kinetic oscillatory instabilities
% and micro-turbulent processes in real-world plasmas.
...
% Laboratory plasmas, however, are usually constrained by specific experimental
% set-up. This specificity would require in-depth knowledge and maybe access to
% one experiment, which makes the analysis in the experimental context restrictive
% and counterproductive. A more broad approach was then to compare our theoretical
% results with phenomena observed in the naturally occurring space plasma.

% \footnote{In the literature, the term ``astrophysical
%   plasma'' is sometimes used to refer to the plasma observed outside the solar
%   system, while ``space plasma'' is used to account for the plasma in our
%   interplanetary space. In this text, however, both terms are being used as
%   synonyms.}

% \footnote{A comprehensive description of the scales and characteristics of
% cosmic plasmas can be seen in Section 1.2 of Ref. \cite{peratt2014}.}
...
Em \cite{klimo} somos apresentados a uma formulação microscópica que combina
os métodos da mecânica clássica para a descrição da variação da posição e do 
momentum das partículas, com as equações de Maxwell para a descrição dos campos 
microscópicos gerados pela movimentação das partículas. Dessa forma, o
estado de um plasma ficaria definido se conseguíssemos determinar as
coordenadas e os momenta de todas as partículas, bem como os valores dos
campos elétrico e magnético em cada ponto, em um determinado instante do
tempo. Com isso, teoricamente, conseguiríamos montar um sistema de equações
que descreveria a evolução temporal do sistema, bastando para isso que
determinássemos as condições iniciais de posição e de momentum para as
partículas e as condições iniciais para os campos.

Contudo, como mencionado anteriormente, essa abordagem é impraticável, devido
a imensa quantidade de partículas que compõem um plasma.
Entretanto, essa formulação pode ser aproximada, permitindo que obtenhamos
uma descrição macroscópica das propriedades do plasma. Existe uma hierarquia
de aproximações que leva às três principais teorias da física de plasmas:
cinética, multi-fluido e magnetoidrodinâmica (MHD). Essas teorias são 
aplicáveis em diferentes contextos, dentro da gama de fenômenos e de condições
que podem ocorrer em um plasma. Como o assunto deste trabalho envolve a
evolução de processos físicos que dependem de detalhes da distribuição de
velocidades das partículas, precisamos aplicar a mais fundamental das três
formulações, a teoria cinética.

A teoria cinética de plasmas consiste na aplicação dos métodos e conceitos da 
mecânica estatística, em conjunto com as equações de Maxwell, para obter uma
descrição macroscópica formal da evolução de um plasma. Este será nosso
ponto de partida.


% This means that a specific charge
% will interact effectively only with other charges that are inside of a
% virtual sphere with radius given by $\lambda_{De}$, being almost completely
% shielded from the influence of particles outside the \emph{Debye sphere}.
% These interactions 
% It is almost because, in the presence of thermal motion, there are small
% fluctuations , creating \emph{Debye shielding}; they are behind the complex
% self-consistent collective behavior of plasmas.