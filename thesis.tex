...
\begin{abstract}
  ...
  (numerical integration), the system seems to arrive at a new quasi-steady
...
of my Masters dissertation, which may be considered the starting point
...
collective modes are not important, and the electric field energy may be
...
The other is the standard situation, in which collective modes are important,
...
was to comprehend important aspects of these so-far unknown expressions,
...
The following paper\footnote{Credit: Reproduced from Physics of Plasmas 23,
  064504 (2016);\url{https://doi.org/10.1063/1.4953802}, with the permission
  of AIP Publishing.} is fairly self-sustained regarding the theory and methods
applied. All important equations are there, including their normalized form
...
\end{split}
 \label{EpsS}
\end{equation}
where $\gamma_{\bf q}^{\sigma L}$ and $\gamma_{\bf q}^{\sigma S}$ are the
dimensionless form of Eqs. \eqref{gammaL} and \eqref{gammaS}, which
describe the collisional damping rate for $L$ and $S$ waves,
respectively
\begin{equation}
  \label{GqL}
  \begin{split}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
The terms $P_{\bf q}^{\sigma L}$ and $P_{\bf q}^{\sigma S}$ are the
normalized versions of Eqs. \eqref{PkL} and \eqref{PkS}, that depict
the electrostatic bremsstrahlung effect for $L$ and $S$ waves, and
are respectively given by
\begin{equation}
  \begin{split}
    P_{\bf q}^{\sigma L}
...
collision integral, which, for $a=e$, is given by
\begin{equation}
  \begin{split}
    \sum_b\theta_{eb}(\Phi_e,\Phi_b) &= \sum_b \Gamma_{eb}
    \left\{\frac{\partial}{\partial{\bf u}}\cdot
     \left(2\frac{m_e}{m_b}\Psi(u_{eb})
     \frac{{\bf u}}{u^3}\Phi_e\right)\right.\\
  &+\frac{\partial}{\partial{\bf u}}\cdot
     \left\{\left[\left(\Phi(u_{eb})
     -\frac{1}{2 u_{eb}^2}\Psi(u_{eb})\right)
     \frac{\partial^2u}{\partial{\bf u}
     \partial{\bf u}}\right]\cdot
     \frac{\partial \Phi_e}{\partial{\bf u}}\right\}
    \label{eq:coll-lin} \\
  &+\left. \frac{\partial}{\partial{\bf u}}\cdot
     \left[\left(\frac{1}{u_{eb}^2}\Psi(u_{eb})
     \frac{{\bf u}{\bf u}}{u^3}\right)\cdot
   \frac{\partial \Phi_e}{\partial{\bf u}}\right]\right\},
\end{split}
\end{equation}
where $u_{eb}\equiv u (v_{t_e}/v_{t_b})$, $u=v/v_{t_e}$,
with $v_{t_e}$ and $v_{t_b}$ representing the thermal velocity of
electrons and thermal velocity of the particles of species $b$,
with $b=e,i$. The quantity $\Psi(x)\equiv\Phi(x)-x\Phi'(x)$ is an
auxiliary function \cite{Gaffey1976}, in which
$\Phi(x) \equiv \frac{2}{\sqrt{\pi}}\int_0^x e^{-t^2}dt$ is the error
function and $\Phi'(x)=\frac{2}{\sqrt{\pi}}e^{-x^2}$ is its derivative.
The factor $\Gamma_{eb}$ is given by $\Gamma_{eb}=2\pi g Z_b^2 \ln\Lambda $,
...
The outcome of this analysis, along with a detailed discussion about
their importance in the context of solar and space physics can be seen
in the following paper\footnote{Credit: S. F. Tigik \emph{et al} 2017
  \emph{ApJL} {\bf 849} L30; \url{https://doi.org/10.3847/2041-8213/aa956f},
...
\begin{split}
\chi_a^{(2)}[{\bf k'},{\bf k'\cdot v'}|{\bf k-k'},({\bf k-k'})\cdot{\bf v}]
=&-\frac{4\pi i e_a}{k k'|{\bf k-k'}|}\frac{1}{2}\int d{\bf v''}\,\biggl[
({\bf k'}\cdot
...
\begin{split}
\chi_a^{(2)}&[{\bf k'},{\bf k'\cdot v'}|{\bf k-k'},({\bf k-k'})\cdot{\bf v}]
=-\frac{2\pi i e_a^3}{k k'|{\bf k-k'}|m_a^2}\int d{\bf v''}\\
...
\begin{split}
\chi_a^{(2)}&[{\bf k'},{\bf k'\cdot v'}|{\bf k-k'},({\bf k-k'})\cdot{\bf v}]
=-\frac{2\pi i e_a^3}{k k'|{\bf k-k'}|m_a^2}\frac{m_a^2}{T_a^2} \int d{\bf v''}\\
...
Due to their asymmetry, the terms which are linear in ${\bf v''}$ will vanish
when integrated, leading to the following expression
\begin{equation}
\begin{split}
\chi_a^{(2)}[{\bf k'},{\bf k'\cdot v'}|{\bf k-k'},({\bf k-k'})\cdot{\bf v}]
&=-\frac{2\pi i e_a^3}{k k'|{\bf k-k'}|m_a^2}\frac{m_a^2}{T_a^2}\\
&\times \int d{\bf v''}\,\left[2\frac{({\bf k'}\cdot{\bf v''})
(({\bf k-k'})\cdot{\bf v''})}
{(\sigma\omega_{\bf k}^L)^2} 
\right] f_a({\bf v''}).
\end{split}
...
&=-\frac{2\pi i e_a^3}{k m_a^2}\frac{m_a^2}{T_a^2}
...
Therefore, the resulting expression for the electrostatic bremsstrahlung
...
For use in this asymptotic equation, let us assume isotropic distributions
for ions and electrons, which are the summation of Maxwellian and Kappa
distributions. In terms of dimensionless variables, we have
\begin{equation}
\label{fbeta}
...
Proceeding in the same way, the expression for the denominator
of the asymptotic wave spectrum becomes
\begin{equation*}
  \begin{split}
   \left(1-\frac{n_{\kappa e}}{n_e}\right)
  &\frac{1}{\pi^{1/2} u_{e}}\frac{1}{q}
...
The equations presented in this appendix and, more important, the ideas
about the asymptotic state of the eigenmodes of an unmagnetized plasma
considering several values of the kappa coefficient, were developed in
parallel with another work, which slightly deviates from the main
subject of this PhD research, but had a big contribution on the
...
\label{appC}
This publication\footnote{Credit: Reproduced from Physics of Plasmas
  24, 112902 (2017);\url{https://doi.org/10.1063/1.5009931}, with the
  permission of AIP Publishing.} is mainly related with the results
presented in Ref.~\cite{Tigik2017a}. It can be said can say that most
of the ideas about the physical processes occurring in~\cite{Tigik2017a},
regarding the formation of the core-halo velocity distribution and the
fact that the electrostatic bremsstrahlung could be the underlying
process behind the ubiquity of such velocity distribution in the space
environment, came from the analysis of the results presented in
\cite{Tigik2017b}. 
...
\label{master}
The paper\footnote{Credit: S. F. Tigik \emph{et al}, A\&A Volume 586,
  February 2016, \url{https://doi.org/10.1051/0004-6361/201527271},
  reproduced with permission, \textcopyright~ESO.} included in this
appendix is the complete text of Ref. \cite{Tigik2016a}, which
summarizes the outcomes of the Masters dissertation \cite{Tigik2015}
that led to the current doctoral research. This text was included
here with the intention to give the reader a complete view of the
work developed during my graduate studies.
